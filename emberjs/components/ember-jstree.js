...
import EmberJstreeActions from '../mixins/ember-jstree-actions';
...
export default Ember.Component.extend(InboundActions, EmberJstreeActions, {
    ...
    */
    _setupJsTree: function() {
        ...
        if(pluginsArray) {
            ...
            if (pluginsArray.indexOf("contextmenu") !== -1 ||
                ...
                configObject["core"]["multiple"] = false;
                configObject["core"]["strings"] = {"Loading ...": " "};
                configObject["dnd"] = { "copy": false };
            }
    ...
    _setupEventHandlers: function(treeObject) {
        ...
        treeObject.on('click', function (e) {
          var target = Ember.$(e.target);
           if(!(target.is('a') || target.parent().is('a'))) {
             treeObject.jstree(true).deselect_all();
             me.sendAction('actionDeselect');
           }
        });

        treeObject.on('loaded.jstree', function () {
           treeObject.jstree('open_all');
        });
